...
const BASE_URL = "http://192.168.5.101:8080";

const mapSelectedValue = (games: Game[]) => {
  return games.map((game) => ({ ...game, label: game.title, value: game.id }));
};
...
const CreateRecord = () => {
  ...
  useEffect(() => {
    axios
      .get(`${BASE_URL}/games`)
      .then((response) => {
        const selectValue = mapSelectedValue(response.data);
